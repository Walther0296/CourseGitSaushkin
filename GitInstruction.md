...
|5.| git push -u origin main         | Подгрузить выполненный проект на github|
|6.| git merge название ветки слияния| Слияние данной ветки с веткой master (выполняется строго из ветки master)|
|7.| git log                         | Вывод коммитов в ветке|
|8.| git log --graph                 | Отображение всех логов в документе (во всех ветках). Используется для поиска ненужных веток|